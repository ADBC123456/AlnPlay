...
- **纯数字剧集识别** — `001.mkv`、`001 - 标题.mkv`、`001_1080p.mkv` 这类以数字开头的文件名会被识别为对应集数（此例为第 1 集）。季号优先取父目录（`Season 2`、`S02`、`第2季`、`第二季`），没有季目录时默认第 1 季。年份（`2024.mkv`）、分辨率（`1080p`）和文件名中间的数字不会被当作集号。媒体库 TMDB 匹配与弹幕批量匹配使用同一套规则，两边得到的季集一致。
- **失败记录自动重试** — 刷新媒体库后，未手动绑定的"需要确认"、"未匹配"以及剧集绑定已失效的文件会按新规则重新匹配；弹幕的自动绑定如果对应的远程剧集已不存在，也会重新映射。手动绑定始终保持不变。
- **媒体库后台刷新** — 启动和回到前台时在后台刷新过期的目录，重复的刷新请求会合并，自动刷新失败后冷却 5 分钟。扫描时文件先入库，TMDB 元数据随后补全。

### 改进

- **弹幕匹配** — 手动选择远程剧集时设置的集数偏移，现在对从文件名解析出的集号同样生效。远程目录中出现重复集号时，不会自动选第一条，而是回退到单文件精确匹配。
- **弹幕加载与缓存** — 弹幕记录统一使用 `danmaku:<resumeKey>` 身份键，旧键在写入时自动迁移去重，手动绑定不会被覆盖。空结果缓存只保留 5 分钟；接口报错、解析失败或条数不一致的响应不再被缓存成空弹幕。一个来源返回空或失败时会继续尝试其他已绑定的来源。强制刷新会贯穿绑定、匹配与缓存各环节，迟到的旧请求不能覆盖新缓存。
- **播放启动更快** — 视频准备和弹幕加载改为并行进行，不再等弹幕加载完才开始播放；切换剧集时，上一集迟到的弹幕结果不会覆盖当前剧集。
- **详情页剧集状态** — 暂时找不到文件的剧集会保留其历史来源，并区分显示"正在更新"、"来源更新失败"和"未找到文件"。
- **首页 Dock 液态玻璃** — 参照 `liquid-glass-react` 的参数重新调校滑块的折射、色散和回弹。光学采样限制在 Dock 自身范围内，不再把 Dock 外的页面也"液化"；滑块经过文字时标签不再跳动。

### 移除

- SMB 与 WebDAV 浏览页右上角的"收藏到首页"按钮。
...
- **AlnPlay visual identity and adaptive appearance** — replaced the former app name and artwork with the supplied AlnPlay logo across Android, iOS/iPad, launchers, notifications, system playback metadata, and the in-app header. Added light, dark, and follow-system appearance modes.
- **Liquid-glass root Dock** — the media library, sources, settings, and search actions now use a responsive floating Dock inspired by AndroidLiquidGlass. Its clipped Gaussian backdrop and local color diffusion keep the soft glass character without pulling readable text from distant parts of the page.
- **Fast episode picker for long series** — the details screen can open a two-column episode sheet beside the season danmaku scrape action, grouped into 50-episode ranges for quick navigation.
...
- **Search remains over the library** — opening search now presents a compact undimmed field over the home screen and lets the keyboard cover the lower shelves, so the library stays visible instead of becoming an empty search page.
- **Resume targets the real episode** — a title's most recently played episode is resolved before recommendations are ordered, centered as the first suggested episode, and selected in the picker instead of leaving the UI positioned at episode 1.
- **Phone and iPad layouts recomposed responsively** — poster sizing, details layouts, Dock width, keyboard behavior, safe areas, landscape, split windows, and large text now adapt across phones and 11-inch iPad Pro layouts without overflow.
- **Settings cleanup** — removed Support, About, version, license, donation, and maker rows from the app settings screen.
