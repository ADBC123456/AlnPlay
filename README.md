...
- 双击画面中间区域可暂停／继续，左右外侧双击分别快退／快进 10 秒；触控锁和画中画下不响应这些手势。
- 每个视频首次加载显示完整准备状态、向前缓冲时长及弹幕匹配、下载、解析进度。开始播放后再缓冲时，仅显示圆环进度与实时速率；正常流畅播放时不显示加载提示。Android Media3 / MPV 使用引擎实际缓冲进度，实时速率为本应用接收速率；无法获取比例或速率时显示“—”，不伪造数值。iOS 暂无视频速率统计。
- 字幕、选集和弹幕设置从右侧半透明面板打开，使用粉色选中态与轻阴影；选集在手机单列、平板双列显示，按实际集号每 50 集分组并定位当前集。
...
- 首页设有“动漫”分组，按元数据的动画类型归入动画电影和动画剧集，不与普通电影、电视剧分组重复；尚无类型信息的历史内容需刷新元数据后归组。
